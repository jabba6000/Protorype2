...
- (IBAction)getForecastButton:(UIButton *)sender {
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Weather For Tommorow"
                                                    message:[DataCollector sharedInstance].weatherTomorrow
                                                   delegate:self
...
- (void)viewDidLoad {
    ...
    [[DataCollector sharedInstance] addObserver:self forKeyPath:@"timeOfRequest" options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:nil];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    self.coordinatesLabel.text = [NSString stringWithFormat:@"%@,\n%@", [DataCollector sharedInstance].longitude, [DataCollector sharedInstance].latitude];
    self.addressLabel.text = [NSString stringWithFormat:@"%@", [DataCollector sharedInstance].address];
    self.weatherLabel.text = [NSString stringWithFormat:@"%@", [DataCollector sharedInstance].weatherToday];
